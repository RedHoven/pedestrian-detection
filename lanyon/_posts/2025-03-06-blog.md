...
Pedestrian detection is a critical task in computer vision with significant real-world implications. With a lot of traffic in European cities, complex crossroads, and growing numbers of road users, protecting pedestrians as the most vulnerable of all traffic users—is a highest priority. Different from cyclists or motorcyclists, who obey traffic regulations somewhat like cars do, pedestrians wander in an irregular manner and hold positions where the drivers least expect them. So, our focus is particularly on pedestrians, not merely as "people" in images, but as a distinct group of traffic that needs to be protected.

Whereas modern-day vehicles increasingly carry all sorts of sensors—e.g., LiDAR and radar—these solutions are costly and hardware-specific. Camera-based pedestrian detection is a low-cost and scalable one. Through the use of computer vision, we can enable smart systems to detect pedestrians from raw visual inputs only, broadening the availability and simplicity of safety technology. For example, pedestrian detection could be the focal part of a broader safety pipeline, like alerting drivers when a pedestrian is behaving unpredictably or suddenly crosses into the road.

But building such systems is not straightforward. Pedestrian detection must work in uncontrolled, real-world settings, from bright afternoons to rainy nights, with partial occlusions, varying clothing, and even adversarial intentional tampering. This leads us to explore how current deep learning models handle these issues and how they can be effectively transferred to new but related domains.
...
The **DETR** model, introduced by [Carion et al. (2020)](#ref-carion), brought transformer-based architectures to object detection, enabling end-to-end detection without the need for hand-designed components like anchor boxes or NMS (non-max suppression). Building on this foundation, **RT-DETR** (Real-Time DETR) aims to retain DETR's benefits while reducing latency for real-time use.
...
TODO: Answer research questions - I would make it a single paragraph 
