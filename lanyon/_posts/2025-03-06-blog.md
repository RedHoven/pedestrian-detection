...
#### Overall Justification for Testing Robustness
...
We have applied all the mentioned augmentations and applied them to 100 images from the testing set - thus generating 5 testset subsets with different augmentations to evaluate and compare the robustness of the finetuned models. The example augmentatios can be seen in Figure bellow.


![alt text](augmented_images.png "Augmentations vizualization")

The implementation details of the augmentations are decribed bellow:
...
The fog was implemented in a following way:
- **White Overlay:**  
...
- **Rain Streaks Generation:**  
  ...
  - For a number of streaks proportional to `rain_intensity`:
    ...
    - Choose a random length (15–25 pixels) and thickness of 2.
    - Find end coordinates
      $
      \text{end\_x} = x + \text{length} \times \sin(\theta), \quad \text{end\_y} = y + \text{length} \times \cos(\theta)
      $
      with $\theta$ between approximately 0.2 and 0.5 radians.
    - Draw a white line representing a rain streak.
...
- **Motion Blur:**  
  ...
  $$
  This spreads the intensity horizontally, simulating falling rain.
...
Overexposure saturates image details. Testing with overexposed images ensures models can detect features even when critical details are lost.
...
- **Gaussian Blur:**  
  Apply a smaller Gaussian blur (5×5) to simulate loss of detail because of the overexposure.


#### 4. Night Simulation
Night scenes have very low brightness and the colors are different. Testing with night-simulated images can evaluate if the low lighting conditions are also acceptable for the model to perform well.
...
- **Blue Tint:**  
  Blend the darkened image with a blue overlay:
  $$
