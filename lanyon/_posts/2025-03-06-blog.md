...
1. How can knowledge from pedestrian detection in road scenes be effectively transferred to other settings and object categories, such as road signs?

2. How do different weather and lighting conditions affect the robustness of pedestrian detection models?
...
To explore the efficiency and effectiveness of transfer learning, we compared two fine-tuning strategies on the traffic sign detection task:
...
We explored how well RT-DETR and YOLO adapt to a new task—**Traffic Sign Detection**—using **full** and **partial fine-tuning** strategies. Below, we compare performance across different validation metrics.
...
This figure compares how each model improves over **training epochs** on four key metrics: **Precision**, **Recall**, **mAP@50**, and **mAP@50-95**.

- **RT-DETR Full Fine-Tuning** consistently outperforms all other setups across all metrics, converging faster and achieving higher final scores.
- **RT-DETR Partial Fine-Tuning** seems to converge faster to its full performance and performs better than both YOLO variants, but only early in training.
- **YOLO Full Fine-Tuning** catches up later but generally underperforms compared to RT-DETR, making it the second-best variation of models over all metrics.
- **YOLO Partial Fine-Tuning** shows the slowest and lowest performance overall, particularly in early epochs. Later on it catches up with the **RT-DETR Partial Fine-Tuning** model.
...
This figure shows the same metrics plotted against **training time (in seconds)** to highlight each model’s **training efficiency**.

- **RT-DETR Full** still leads in performance and again converges the fastest.
- **RT-DETR Partial** shows promising results at the beginning, but after around 3000 seconds it becomes the least performing algorithm.
- **YOLO models** show comparable performance with respect to their training time. The partially fine-tuned version even slightly outperforming the other one during big ranges of time and across all of the metrics.
