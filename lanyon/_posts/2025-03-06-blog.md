...
Pedestrian detection models can be sensitive to variations in image data, including changes in lighting, weather conditions. We evaluate how these models perform in different weather and light conditions to evaluate their robustness. We also propose ways in which the models could be improved in the future.

### 2. Transfer Learning

Training deep learning models from scratch is computationally expensive. Instead of training from scratch, we applied transfer learning using YOLOv8 and RT-DETR models pre-trained on the COCO dataset. These models were first fine-tuned for pedestrian detection and later adapted to traffic sign detection—both tasks set in urban environments. We compared **full fine-tuning**, **partial fine-tuning**, and using **Ultralytics' default COCO weights**, to evaluate how well these models transfer across related domains with minimal compute and data.
### 3. Explainability
...
1. How do different weather and lighting conditions affect the robustness of pedestrian detection models?

2. Can the knowledge from pedestrian detection in road scenes be effectively transferred to other settings and object categories, such as road signs?

3. What are the differences in the influential image regions that YOLOv8 and RT-DETR rely on for pedestrian detection across levels of crowding and occlusion?
...
Pedestrian detection is an important component in the development of autonomous driving systems, traffic monitoring, and urban surveillance. Over the years, various models have been proposed to enhance the accuracy and efficiency of detecting pedestrians in complex urban environments. Among these, the You Only Look Once (YOLO) series and the Detection Transformer (DETR) models have garnered significant attention.

### Evolution of YOLO in Pedestrian Detection

The YOLO series has been instrumental in advancing real-time object detection. YOLOv5, for instance, introduced a balance between speed and accuracy, making it suitable for various applications. Building upon this, researchers developed the **YOLOv5-FFM** model, which incorporates a Feature Fusion Module (FFM) to address the challenges posed by occluded pedestrians. This enhancement led to improved detection accuracy with reduced computational demands, specifically targeting occlusion scenarios ([source](https://arxiv.org/abs/2408.06633)).

The subsequent iteration, **YOLOv8**, further refined detection capabilities by integrating advanced deep learning techniques. A study focusing on real-time detection and analysis of vehicles and pedestrians employed YOLOv8, demonstrating its proficiency in processing complex visual inputs across diverse environmental conditions ([source](https://arxiv.org/abs/2404.08081)).

### Advancements with DETR and RT-DETR

The introduction of the **Detection Transformer (DETR)** marked a paradigm shift in object detection by leveraging transformer architectures. Building upon DETR, the **Real-Time Detection Transformer (RT-DETR)** was designed to enhance real-time detection performance. **FedsNet**, an RT-DETR-based network, showcased a 1.7% accuracy improvement in pedestrian detection while achieving a 15.1% reduction in computational complexity, highlighting its efficiency and effectiveness ([source](https://www.researchgate.net/publication/382653528_FedsNet_the_real-time_network_for_pedestrian_detection_based_on_RT-DETR)).

### Rationale for Selecting YOLOv8 and RT-DETR

In our research, we have chosen to utilize **YOLOv8** and **RT-DETR** models for the following reasons:

1. **Performance and Efficiency**:
   Both models have demonstrated superior accuracy in pedestrian detection tasks while maintaining real-time processing capabilities. Their architectures are optimized to handle the complexities of urban environments effectively.

2. **Adaptability to Environmental Variations**:
   Studies have shown that YOLOv8 and RT-DETR can process complex visual inputs across diverse environmental conditions, making them suitable for real-world applications where lighting and weather conditions may vary.

3. **Architectural Innovations**:
   YOLOv8 benefits from continuous improvements in the YOLO series, incorporating advanced deep learning techniques. RT-DETR leverages transformer-based architectures, offering a novel approach to object detection that enhances both speed and accuracy.

By employing these models, our research aims to build upon existing advancements to further enhance pedestrian detection systems in urban settings.


[//]: # (## Literature Review)

[//]: # ()
[//]: # (Pedestrian detection has evolved significantly with deep learning, transitioning from traditional methods &#40;HOG+SVM, DPM&#41;. Two-stage detectors like Faster R-CNN &#40;Ren et al., 2015&#41; introduced Region Proposal Networks &#40;RPNs&#41; **cite?,** significantly improving accuracy. However, their computational cost led to the rise of one-stage detectors such as **YOLO** full name &#40;Redmon et al., 2016&#41; and **SSD** again full name , which prioritize speed while maintaining accuracy.)
...
[//]: # ()
[//]: # (Pedestrian detection datasets cover a wide range of conditions, from dense urban environments to low-light and thermal imaging. Below is a brief overview of key benchmarks and their top-performing models:)
...
[//]: # ()
[//]: # (These datasets highlight specific challenges—such as occlusion, night-time conditions, and modality shifts—and the associated models represent state-of-the-art solutions for each context.)
...
(here they user RL-DETR-L, we are going to use RT-DETR_r50vd)
...
We compare the predictive performance of `YOLOv8l` and `RT-DETR-L`, using standard evaluation metrics(precision, recall, F1-score, and mean average precision (mAP) at different intersection over union (IoU)thresholds). The purpose of this comparison is to evaluate how well each model performs in detecting pedestrians in different cities, with a focus on localization accuracy and the trade-off between precision and recall. The results are presented in Table 1.

_Table 1: A comparison of `YOLOv8l` and `RT-DETR-L` models using standard object detection evaluation metrics: precision, recall, f1-score and mAP for different IoU thresholds. The best result per metric is shown in bold._
...
We have applied all the mentioned augmentations and applied them to 100 images from the testing set - thus generating 5 testset subsets with different augmentations to evaluate and compare the robustness of the finetuned models. The example augmentation can be seen in Figure X.

!["Augmentations vizualization"]({{ '/public/images/augmented_images.png' | relative_url }})

_Figure X: Example augmentations applied to images to simulate fog, rain, overexposure, and night-time conditions._
...
To investigate the generalization capabilities of pedestrian detection models, we applied **transfer learning** to a related but distinct task: **traffic sign detection**. This experiment allows us to evaluate how well models trained on one type of object (pedestrians) can adapt to detecting a different, yet equally important category (traffic signs). Both tasks are critical components in autonomous driving systems and share similarities such as visual clutter, small object sizes, and real-time detection constraints. Another similarity is that both datasets have the same visual context (e.g., road layout, signage, vehicles, pedestrians), and it will be interesting to see whether this actually help the models to generalize.

For this purpose, we used the publicly available dataset from: [Traffic Sign Detection](https://universe.roboflow.com/selfdriving-car-qtywx/self-driving-cars-lfjou).
...
This dataset is ideal for training models in use cases such as autonomous vehicle navigation, traffic rule compliance, road safety training, smart city infrastructure, and road network analysis.
...
- **Full Fine-Tuning**: All layers of the pre-trained model are updated during training.
- **Partial Fine-Tuning**: Only the final layers are updated, while the backbone layers remain frozen.
...
Including the default-weight models is important, as it allows us to assess whether the general-purpose features learned from COCO are already sufficient for strong performance in a related domain, and whether the training in the same setting can be an advantage.
...
The results shows that the robustness of RT-DETR and YOLO is environmentally conditioned. (See Figure X) Under normal, well-lit conditions, RT-DETR outperforms YOLO on nearly all metrics, including mAP, precision, and recall, suggesting better detection accuracy under good visibility conditions. Under foggy weather, although both models have comparable mAP@0.50 scores, YOLO has a slight advantage over RT-DETR on stricter metrics (mAP@0.50–0.95, mAP@0.75, precision, and recall), which can be possibly explained by the fact that YOLO's feature extraction is more capable of dealing with low-contrast images. Under rainy conditions, RT-DETR performs better with much higher mAP values and better precision and recall, showing a strong ability to deal with image distortions and occlusions like rain strokes. Overexposed settings show little variation between models, each with marginal superiority in various metrics, suggesting equal performance under high exposure. In night or low-light settings, YOLO is more robust and outperforms RT-DETR in every metric. While RT-DETR is more robust in rain and well-lit scenes and performs better, YOLO is the more stable model under adverse weather conditions such as fog and especially in low-light or night-time conditions.
...
_Figure X: Robustness comparison between RT-DETR and YOLO under various environmental conditions, including fog, rain, overexposure, and night-time scenarios._
...
- *A lightweight YOLOv5-FFM model for occlusion pedestrian detection*. [arXiv:2408.06633](https://arxiv.org/abs/2408.06633)
- *Real-Time Detection and Analysis of Vehicles and Pedestrians using Deep Learning*. [arXiv:2404.08081](https://arxiv.org/abs/2404.08081)
- *FedsNet: The real-time network for pedestrian detection based on RT-DETR*. [ResearchGate](https://www.researchgate.net/publication/382653528_FedsNet_the_real-time_network_for_pedestrian_detection_based_on_RT-DETR)
